...
function enviarCita() {
  ...
  fetch('/api/cita', {
    ...
    body: JSON.stringify({ nombre, email, fecha, hora, motivo })
  })
    .then(res => res.json())
    .then(data => {
      ...
      if (data.ok) {
        ...
        errorEl.style.color = '#e74c3c';
        errorEl.textContent = '⚠️ ' + (data.error || 'Error al enviar.');
        errorEl.style.display = 'block';
      }
    })
    .catch(() => {
      ...
      btnEnviar.textContent = '✉️ Enviar solicitud de cita';
      errorEl.style.color = '#e74c3c';
      errorEl.textContent = '⚠️ Error de conexión. Inténtalo de nuevo.';
      errorEl.style.display = 'block';
    });
}
